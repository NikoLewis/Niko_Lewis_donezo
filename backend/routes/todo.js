    ...
    try {
        ...
        const newTodo = await prisma.todo.create({
            data: {
                ...
                //Niko note: comment out below line  when testing post prior to middleware
               userId: req.user.sub, // Assign the user ID
               //Niko note: uncomment the below line when testing post prior to middleware
...
router.delete("/:todoId", async (req, res) => {
  // Extract the `todoId` from the route parameter and convert it to a number
  const todoId = Number(req.params.todoId);
  const todoCompletionStatus = Boolean(req.params.completed)
  ...
  try {
    ...
    await prisma.todo.delete({
      where: {
        id: todoId,
        completed: true // Match the todo based on its unique ID'
      },
    });
